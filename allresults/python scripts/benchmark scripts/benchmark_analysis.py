    ...
    for file in files:
        if file.startswith("availability_times") and file.endswith(".csv"):
            ...
            if len(latencies) == 0:
                ...
                print(f" column found: {df.columns.tolist()}")
                print(f"   total number of rows in the csv: {len(df)}")
                print(f"   non null values in availability times: {df['availability_time'].count() if 'availability_time' in df.columns else 'missing col'}")
                continue
            else:
...
print("first rows df_avail:")
print(df_avail.head())
print("first rows df_lat:")
print(df_lat.head())
print("========================\n")
